...
func find(data []core.Score, id uint32) (bool, int) {
	left := 0
	right := len(data) - 1
...
func (f *FastSort) Add(scores []*core.Score) {
	 if scores == nil {
		 return
	 }
	 f.Lock()
...
func (f *FastSort) GetSort()  []core.Score {
	 var result = make([]core.Score,len(f.data))
